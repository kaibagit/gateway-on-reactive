 ...
 */
@Slf4j
public class ValidateProcessor implements Processor<ApplicationQa,ApplicationQa> {
    ...
    public Mono<ApplicationQa> process(RequestContext ctx, Mono<ApplicationQa> mono) {
        return mono.flatMap(applicationQa -> {
            ...
            if(!generatedSign.equals(requestParam.getSign())){
                Pair<String,String> unsignStringAndFinalSign = Signer.getUnsignStringAndFinalSign(requestParam.getAppkey(), requestParam.getTimestampStr(), requestParam.getNonce(), requestParam.getAccess_token(), requestParam.getApi(),applicationQa.getAppSecret(),requestParam.getBiz_param());
                log.info("签名失败， 签名前字符串：{} 最后签名：{}",unsignStringAndFinalSign.getLeft(),unsignStringAndFinalSign.getRight());
                return Mono.error(new OpenGatewayException(CodeEnum.SYS_INVALID_SIGNATURE));
            }
